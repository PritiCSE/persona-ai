...
export const TopNav = ({ title, subtitle }: { title: string; subtitle?: string }) => (
  <header className="sticky top-0 z-20 h-16 border-b border-border/60 bg-background/80 backdrop-blur-xl px-6 flex items-center gap-4">
    ...
    <div className="hidden md:flex items-center gap-2 h-9 px-3 rounded-xl border border-border bg-input/40 text-sm text-muted-foreground w-72 hover:border-border-strong transition">
      <Search className="h-4 w-4" />
      <span className="flex-1">Search prospects, campaigns...</span>
