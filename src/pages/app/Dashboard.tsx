...
const Dashboard = () => {
  ...
  const handleApplyInsight = async () => {
    setApplying(true);
    try {
      const response = await fetch("http://localhost:8000/api/v1/campaigns/1/apply-insight", {
        method: "POST"
      });
      const data = await response.json();
      toast({
        title: "Success",
        description: data.message || "Insight applied to campaign",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to apply insight",
        variant: "destructive"
      });
    } finally {
      setApplying(false);
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-4">
                  <button
                    ...
                    disabled={applying}
                    className="text-xs font-medium px-3 py-2 rounded-lg gradient-primary text-primary-foreground hover:opacity-90 inline-flex items-center gap-1.5 disabled:opacity-50">
                    {applying ? "Applying..." : "Apply to campaign"} <ChevronRight className="h-3.5 w-3.5" />
                  </button>
